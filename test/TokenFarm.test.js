...
contract('Token Farm', ([owner, investor]) => {
    ...
    describe('Stake Tokens', async () => {
        it('Investor must has 1000 DAI tokens before staking', async () => {
            ...
            assert.equal(investorBalance, tokens('1000'), 'Investor must has 1000 dai tokens');
        });
        ...
        it('Investor balance must be 900 DAI tokens after staking', async () => {
            ...
            //check investor balance of dai tokens afer staking
            let updatedInvestorBalance = (await daiToken.balanceOf(investor)).toString();
        ...
        it('Token farm must has 100 dai tokens', async () => {
            let tokenFarmBalance = (await daiToken.balanceOf(tokenFarm.address)).toString();
